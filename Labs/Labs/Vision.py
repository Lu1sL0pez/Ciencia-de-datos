def show_image_caption(image_path, analysys):
    ...
    caption_text = ''
    if (len(analysys.captions) == 0):
        caption_text = 'No caption detected'
    else:
        for caption in analysys.captions:
...
def show_image_analysis(image_path, analysis):
    ...
    # Get the caption
    caption_text = ''
    if (len(analysis.analysys.captions) == 0):
        caption_text = 'No caption detected'
    else:
        for caption in analysis.analysys.captions:
            caption_text = caption_text + " '{}'\n(Confidence: {:.2f}%)".format(caption.text, caption.confidence * 100)
    plt.title(caption_text)
